...
/**
 * EditProgressItemForm Component
 *
 * Form for editing existing progress items.
 */
import { ref, computed, watch } from 'vue'
import { Button, Input, Card, FormField } from '@/components/ui'
import type { ProgressItem, UpdateProgressItemDto, DayOfWeek, Importance, Urgency } from '../types/progress.types'
...
const emit = defineEmits<{
  success: [dto: UpdateProgressItemDto]
  cancel: []
}>()

// Form state - initialize from props
const title = ref(props.item.title)
const importance = ref<Importance>(props.item.importance)
const urgency = ref<Urgency>(props.item.urgency)
const activeDays = ref<DayOfWeek[]>([...props.item.activeDays])
...
// Watch for prop changes (in case item is refreshed)
...
async function handleSubmit() {
  ...
  try {
    ...
    validationError.value = err.response?.data?.message || 'Failed to update progress item'
  } finally {
...
<template>
  <Card variant="default" padding="md">
    <h2 class="text-xl font-semibold text-neutral-900 mb-4">Edit Progress Item</h2>
